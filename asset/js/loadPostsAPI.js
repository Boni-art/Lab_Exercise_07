...
function searchUsers() {
    ...
    fetch('https://jsonplaceholder.typicode.com/posts')
        ...
        .then(function(posts) {
            //iterate over each post [100 posts]
            let toBeSearched = Array();
            posts.forEach(function(post) {
                toBeSearched.push({title:post.title.toUpperCase(), body: post.body});
            });
            let output = '';
            for (let i = 0; i < toBeSearched.length; i++) {
                const searchContent = search.value.toUpperCase();
                                
                if (toBeSearched[i].title.toUpperCase().indexOf(searchContent) > -1) {
                ...
                <div class="content">
                    <a class="header" href="#" id="bTitle">
                    ${toBeSearched[i].title.toUpperCase()}
                    </a>
                    <div class="description">
                        <p id="bDesc">
                        ${toBeSearched[i].body}
